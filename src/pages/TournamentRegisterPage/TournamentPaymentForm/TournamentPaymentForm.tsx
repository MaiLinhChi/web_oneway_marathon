...
const TournamentPaymentForm: React.FC<TTournamentPaymentFormProps> = () => {
  ...
  const orderDetailLoading = useSelector((state: any) => state.loadingReducer[EGetOrderByIdAction.GET_ORDER_BY_ID]);
  const paymentMethodLoading = useSelector(
    (state: any) => state.loadingReducer[EGetPaymentMethodAction.GET_PAYMENT_METHOD],
  );
  ...
  return (
    <>
      {orderDetailLoading || paymentMethodLoading ? (
        <Loading />
      ) : (
          ...
          <Form layout="vertical" form={form} onFinish={handleSubmit}>
            <div className="TournamentRegisterPage-card">
              ...
              >
                ...
                <ol>
                  <li>
                    1. Việc tiến hành thanh toán suất BIB là bước tiếp theo sau khi vận động viên đã đọc và đồng ý tất
                    cả nội dung của giải chạy; về qui định, qui chuẩn của vận động viên tham gia giải chạy. Vận động
                    viên cam kết không có bất kỳ khiếu nại nào về việc không đủ tiêu chuẩn để tham gia mặc dù đã thanh
                    toán.
                  </li>
                  <li>
                    2. Số báo danh thi đấu (BIB) dành riêng cho mỗi người tham dự và không thể thay đổi. Việc tự ý mua
                    bán/hoặc chuyển nhượng BIB bị nghiêm cấm, kết quả sẽ không được công nhận khi bị phát hiện người
                    chạy sai tên, sai BIB, sai thông tin đã đăng ký.
                  </li>
                  <li>
                    3. Về chính sách ghi nhận thanh toán hoàn tất và chốt vé cho khách: sau khi ghi nhận thông tin đăng
                    ký thì Oneway sẽ gọi điện hay email để xác nhận; trong vòng 24h giờ quý khách cần thanh toán để chốt
                    vé.{' '}
                  </li>
                  <li>
                  ...
                  <li>
                    5. Trong trường hợp muốn thay đổi BIB cự ly cần thực hiện trước ít nhất 1 tuần so với ngày diễn ra
                    giải chạy. Việc xác nhận thông tin mới sẽ được cập nhật thông qua email xác nhận của Oneway và việc
                    thay đổi cự ly hoàn toàn miễn phí. Vận động viên chỉ cần thanh toán phí chênh lệch theo giá trị của
                    các cự ly, với cự ly nhỏ hơn sẽ không hoàn phí chênh lệch.
                  </li>
                  <li>
                    6. Điều khoản bảo mật thông tin : Onepay không lưu giữ thông tin thẻ của Khách hàng, thông tin chỉ
                    dùng để thực hiện thanh toán và dùng trong trường hợp cần thiết đối chiếu với thông tin của vận động
                    viên.{' '}
                  </li>
                  <li>
                    7. Trách nhiêm và quyền hạn của onepay đối với sự kiện bất khả kháng: Chúng tôi có quyền hủy đặt chỗ
                    hoặc thay đổi dịch vụ bất kỳ lúc nào khi phát sinh các trường hợp không lường trước được. Những
                    trường hợp như vậy bao gồm nhưng không giới hạn ở: chiến tranh hoặc đe dọa chiến tranh, lây lan dịch
                    bệnh, bạo loạn, xung đột dân sự, hoạt động khủng bố, tranh chấp công nghiệp, thảm họa tự nhiên hoặc
                    hạt nhân, hỏa hoạn, thời tiết bất lợi và / hoặc điều kiện giao thông, hành động của chính phủ hoặc
                    đối với bất kỳ lý do nào nằm ngoài tầm kiểm soát của chúng tôi. Trong những trường hợp này, chúng
                    tôi không chịu trách nhiệm pháp lý khi giải chạy buộc phải hủy bỏ hoặc thay đổi do bất kỳ trường hợp
